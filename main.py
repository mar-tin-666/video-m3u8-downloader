...
def download_m3u8(m3u8_url_inner, output_filename_inner):
    """Downloads and merges video segments from an .m3u8 playlist"""
    response = requests.get(m3u8_url_inner, timeout=10)
    response.raise_for_status()
    ...
    base_url = os.path.dirname(m3u8_url_inner)  # Base URL for segments
    segment_urls = [
        segment.uri if segment.uri.startswith("http") else f"{base_url}/{segment.uri}"
    ...
    ffmpeg.input(concat_list_path, format="concat", safe=0).output(
        output_filename_inner + ".mp4", c="copy"
    ).run(overwrite_output=True)
    print(f"Video saved as: {output_filename_inner}.mp4")
...
if __name__ == "__main__":
    user_m3u8_url = input("Enter the .m3u8 URL: ")
    user_output_filename = input("Enter the output filename (without extension): ")

    if not validate_filename(user_output_filename):
        print(
            "Error: The filename can only contain letters and numbers"
            " (no spaces or special characters)."
        )
    else:
        download_m3u8(user_m3u8_url, user_output_filename)
